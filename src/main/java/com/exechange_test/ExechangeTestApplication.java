    ...
    public static void main(String[] args) throws Exception {
        ...
        SimpleEventsProcessor eventsProcessor = new SimpleEventsProcessor(new IEventsHandler() {
            ...
            public void tradeEvent(TradeEvent tradeEvent) {
                ...
                // For each trade, check if there are any Stop Loss orders to activate
                for (Order order : stopLossOrders) {
                    if (tradeEvent.getPrice() <= order.getTriggerPrice()) {
                        // If market price reaches the trigger price, activate Stop Loss order
                        order.setPrice(order.getFinalPrice());
                        api.submitCommandAsync(ApiPlaceOrder.builder()
                                .uid(order.getUid())
                                .orderId(order.getOrderId())
                                .price(order.getFinalPrice())  // Set the final price for sell
                                .size(order.getSize())
                                .action(OrderAction.ASK)  // It is a sell order
                                .orderType(OrderType.GTC)  // Normal order after activation
                                .symbol(order.getSymbol())
                                .build());
                        System.out.println("Stop Loss order activated: " + order);
                    }
                }
            }
